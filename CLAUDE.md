...
- **The title bar (`#uf-topbar`, order -2) collapses on scroll; the pane switcher (`#uf-pane-switch`, order -1) stays pinned below it.** `App._setupHeaderAutoHide()` runs one capturing `scroll` listener over all inner scrollers (CM `.cm-scroller`, preview, commit log) and toggles `#unifile-app[data-header-hidden]`: scroll down → hide (`#uf-topbar` height→0), scroll up / near-top → reveal. iOS-style collapsing nav; reclaims the header height for content. Header is force-revealed on pane switch and pinned open during a commit diff (`[data-diff]`). The **safe-area inset is on `#unifile-app` itself** (`padding-top: env(safe-area-inset-top)` + `background:var(--bg-alt)`, border-box keeps `--app-height`), NOT on the collapsing top bar — otherwise the inset would strand a gap when the bar collapses. The pane switcher rail is a **compact 38px** (icons 19px).

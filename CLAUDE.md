...
The app is a `100dvh`-ish flex column: (site-nav) · top bar · **pane switcher** · `#uf-main` · `#uf-bottom`. On phones (`@media max-width:640px`) only the active one of three panes (**commit-log · editor · render**) is displayed. `App._setupMobilePanes()` tracks the pane into `#unifile-app[data-mobile-pane]` and drives per-pane UI. The **top bar (`#uf-topbar`, order -2) auto-hides on scroll**; the **pane switcher (`#uf-pane-switch`, order -1) stays pinned** directly below it. The safe-area inset lives on `#unifile-app` (padding-top) so the collapsing top bar never strands it.
...
- **The title bar (`#uf-topbar`, order -2) collapses on scroll; the pane switcher (`#uf-pane-switch`, order -1) stays pinned below it.** `App._setupHeaderAutoHide()` runs one capturing `scroll` listener over all inner scrollers (CM `.cm-scroller`, preview, commit log) and toggles `#unifile-app[data-header-hidden]`: scroll down → hide (`#uf-topbar` height→0), scroll up / near-top → reveal. iOS-style collapsing nav; reclaims the header height for content. Header is force-revealed on pane switch and pinned open during a commit diff (`[data-diff]`). **Hardened against iOS bounce** (each fixed a real jitter): (1) frozen while the editor/an input is focused (`_editing`) so caret-tracking scrolls + keyboard viewport resizes never toggle it while typing; (2) a post-toggle **cooldown** swallows the collapse's own reflow-scroll (the feedback loop); (3) a **direction accumulator + hysteresis** (hide past ~40px down, show past ~24px up) instead of reacting to per-event deltas. The **safe-area inset is on `#unifile-app` itself** (`padding-top: env(safe-area-inset-top)` + `background:var(--bg-alt)`, border-box keeps `--app-height`), NOT on the collapsing top bar — otherwise the inset would strand a gap when the bar collapses. The pane switcher rail is a **compact 38px** (icons 19px).
- **Mobile top bar is identity-only** (`[☰ menu] · centred title · • dirty dot`). Branch/commit chrome moved OFF the top bar into the commit pane. The dirty dot (`.tb-dirty`, mobile-only) shows only when uncommitted; tap → jumps to the commit pane. Desktop keeps the `.topbar-right` VCS pills.
